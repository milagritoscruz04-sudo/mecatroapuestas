...
ESP32_IP = "http://192.168.1.15"  # Reemplazar con la IP asignada a su ESP32
...
def init_db():
    ...
    if not cursor.fetchone():
        cursor.execute("INSERT INTO usuarios VALUES (?, ?, ?, ?)", ('M000', 'admin', 'admin123', 5000.0))
...
def generar_siguiente_id():
    ...
    conn.close()
    return f"M{total:03d}"  # Genera M001, M002, M003...
...
# Función para enviar orden al ESP32 en segundo plano (Evita que la web se quede colgada)
...
def calcular_ganador_casa(apuestas_activas):
    ...
    # 30% de probabilidad: Aleatorio total
    if random.random() < 0.30:
        return random.randint(0, 23)
        
    # 70% de probabilidad: Favorecer la casa (Elegir un número con menor/ninguna ganancia otorgada)
...
def login_view():
    ...
    accion = data.get('action', 'login') # 'login' o 'register'
...
# PANEL ADMIN PARA USTEDES LOS CREADORES
@app.route('/admin')
def admin_panel():
    ...
    historial = cursor.execute('SELECT * FROM historial ORDER BY id DESC LIMIT 30').fetchall()
    conn.close()
...
def apostar_individual():
    ...
    cursor.execute('UPDATE usuarios SET saldo = ? WHERE id = ?', (nuevo_saldo, session['user_id']))
    cursor.execute('''
    ...
    # Enviar comando en hilo separado
